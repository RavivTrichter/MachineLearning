...
def read_data(filename_x, filename_y, flag = False):
    ...
    for line in x_file:
        ...
        x.append([1.0, float(tmp_x_values[0]), float(tmp_x_values[1]), float(tmp_x_values[0])*float(tmp_x_values[0])])
...
def draw_data(theta=None):
     ...
     if theta is not None: # drawing the line to seperade the data
         ...
         pts_y = np.array([-(theta[0] + (theta[1]*p)+ (theta[3]*(p**2)) )/theta[2] for p in pts_x])
         plt.scatter(pts_x,pts_y)
         plt.savefig('sec2_quadratic.png')
     else:
...
if __name__ == "__main__":
    ...
    print('theta : ', theta)
    plt.plot(Jiter)
    plt.savefig('Jiter_section2.png')
    plt.show()
    draw_data(theta) # with quadratic line
    x = []
    y = []
